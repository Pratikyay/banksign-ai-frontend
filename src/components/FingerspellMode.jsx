    ...
    <div className="min-h-screen bg-navy-50">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 flex flex-col items-center gap-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-navy-900">Fingerspell Mode</h1>
          <p className="text-navy-500 text-sm">Sign letters to build your sentence</p>
        </div>
        ...
        <div className="text-center">
          <div className="text-6xl font-bold text-gold-500">{currentLetter}</div>
          <p className="text-navy-500 text-xs mt-1">Confidence: {(confidence * 100).toFixed(1)}%</p>
        </div>

        <div className="w-full max-w-lg bg-white border border-navy-100 rounded-xl p-4 min-h-[60px]">
          <p className="text-navy-300 text-xs mb-1">Built sentence:</p>
          <p className="text-lg text-navy-900 font-medium break-words">{sentence || <span className="text-navy-300 italic">Start signing…</span>}</p>
        </div>

        <div className="flex gap-3">
          {Object.entries(actions).map(([key, label]) => (
            <button key={key} onClick={() => applyAction(key)} className="px-4 py-2 bg-white border border-navy-100 rounded-lg text-sm font-medium text-navy-600 hover:bg-navy-50 transition">{label}</button>
          ))}
        </div>
        ...
        <div className="bg-white border border-navy-100 rounded-xl p-4 w-full max-w-lg text-sm text-navy-500">
          <p className="font-semibold text-navy-900 mb-2">Gesture Instructions</p>
